...
for (d in d_values) {
  ...
  params_fkf <- list(
    dt = matrix(0, d, 1), ct = matrix(0, d, 1), Tt = as.matrix(diag(1, d)),
...
for (rep_id in 1:n_repeats) {
  message("Running ORC Simulation Repeat: ", rep_id)
...
#### experiment for bpf ####
for (rep_id in 1:n_repeats) {
  message("Running BPF Simulation Repeat: ", rep_id)
...
#### experiment for csmc ####
for (rep_id in 1:n_repeats) {
  message("Running CSMC Simulation Repeat: ", rep_id)
...
log_diff_result_clean <- log_diff_result[log_diff_result != 0]

obs_svm  <- as.matrix(100*log_diff_result_clean)
...
for(d_ in d_values){
  ...
  for (i in 1:num_repeats) {
    ...
    results_list[[length(results_list) + 1]] <- data.frame(
      x = compute_ratio(output_bpf$logZ, fkf_logZ), 
      method = "bpf", 
      d = d_
    )
...
diag <- do.call(rbind, results_list)

diag <- diag[, c("x", "method", "d")]

write.csv(diag, "adaptive_orc_bpf_10000T100d2-64.csv", row.names = FALSE)
...
tikz("neuro_logz.tex", width = 3.3, height = 3.2, sanitize = FALSE)
print(p_logz)
dev.off()
