...
adaptative_Orc_SMC <- function(B_max, K_min, K_max, K1, gamma, data, model, N1) {
  ...
  psi_pa <- matrix(NA, nrow = T_total + 1, ncol = 2 * d) 
  ...
  for (t in 1:T_total) {
    ...
    out_init <- run_psi_APF_rolling(data, t, 
                                    psi_t = psi_pa[t, ], 
                                    H_prev = H_prev_trial, 
                                    model, 
                                    init = TRUE, 
    ...
    if (Kt > 0) {
      for (k in 1:Kt) {
        ...
        #concert previous psi_pa into coefficients matrices
        curr_block_coeff <- t(apply(psi_pa[t0:t, , drop=FALSE], 1, psi_to_coeff))
    ...
    for (b in 1:Bt) {
      ...
      #convert it to coefficient matrices
