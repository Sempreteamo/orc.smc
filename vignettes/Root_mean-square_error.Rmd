---
title: "Reproducing ORCSMC Experiments (Figure 3)"
...
This section focuses on the reproduction of **Figure 3**, which illustrates the relationship between the rolling window length (Lag) and the approximation error, demonstrating that the Root Mean Square Error (RMSE) of the normalizing constant estimates decays as the lag increases across all considered dimensions.
  ...
  for (current_lag in lag_list) {
    #cat("  Testing Lag =", current_lag, "\n")
     ...
     all_results <- rbind(all_results, data.frame(
        Dimension = as.numeric(d_),
        Lag = as.numeric(current_lag),
        Ratio = ratio_val
      ))
...
--- Compute RMSE and Plot (Replicating Figure 3) ---
```{r}
# Calculate RMSE per Dimension and Lag
rmse_summary <- all_results %>%
  group_by(Dimension, Lag) %>%
  summarize(
    # RMSE = sqrt( mean( (Ratio - 1)^2 ) )
    # Since the ideal ratio is 1.0
    RMSE = sqrt(mean((Ratio - 1)^2)),
    .groups = 'drop'
  )
```
...
```{r}
ggplot(rmse_summary, aes(x = Lag, y = RMSE, color = factor(Dimension), shape = factor(Dimension))) +
  geom_line() +
  geom_point(size = 3) +
  labs(
    title = "Figure 3: RMSE of Relative Normalising Constant Estimate",
    subtitle = "Approximation error decays with increasing Lag (L)",
    x = "Lag",
    y = "RMSE",
    color = "Dimension",
    shape = "Dimension"
  ...
  theme_bw() +
  scale_x_continuous(breaks = lag_list)
```
